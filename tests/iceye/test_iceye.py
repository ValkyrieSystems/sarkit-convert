...
def test_polyfit2d():
    ...
    coefs = sarkit_convert.iceye.polyfit2d(x.flatten(), y.flatten(), z.flatten(), 2, 3)
...
def test_polyfit2d_invalid_inputs(x, y, z, error_msg):
    with pytest.raises(ValueError, match=error_msg):
        sarkit_convert.iceye.polyfit2d(x, y, z, 1, 1)
...
def test_fit_state_vectors_nom(data):
    np.testing.assert_allclose(
        data["position_poly"],
        sarkit_convert.iceye.fit_state_vectors(
...
def test_broadening_from_amp_smoke():
    amps = np.ones(1024)
    broadening = sarkit_convert.iceye.broadening_from_amp(amps)
